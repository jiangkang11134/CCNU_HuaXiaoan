...
from server.utils.frontend_portal_policy import (
    ensure_frontend_agent_allowed,
    ensure_frontend_run_allowed,
    get_frontend_allowed_agent_ids,
    get_frontend_chat_config,
    is_frontend_chat_user,
)
...
async def list_agents(
    ...
    if is_frontend_chat_user(current_user):
        allowed_agent_ids = get_frontend_allowed_agent_ids(await get_frontend_chat_config(db))
        items = [item for item in items if item.slug in allowed_agent_ids]
...
async def get_agent(agent_id: str, current_user: User = Depends(get_required_user), db: AsyncSession = Depends(get_db)):
    await ensure_frontend_agent_allowed(db, current_user, agent_id)
    repo = AgentRepository(db)
...
async def delete_agent(
    agent_id: str, current_user: User = Depends(get_admin_user), db: AsyncSession = Depends(get_db)
):
    repo = AgentRepository(db)
    agent_slug = agent_id  # 兼容既有路径参数名；这里实际是 Agent.slug。
    item = await repo.get_visible_by_slug(slug=agent_slug, user=current_user, kind="any")
...
async def create_agent_run(
    ...
    await ensure_frontend_run_allowed(
        db,
        current_user,
