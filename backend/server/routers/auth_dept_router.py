...
async def get_department(
    department_id: int, current_user: User = Depends(get_admin_user), db: AsyncSession = Depends(get_db)
):
    """获取指定部门详情"""
...
async def create_department(
    ...
    # 记录操作
    await log_operation(db, current_user.id, "创建部门", f"创建部门: {department_data.name}", request)
