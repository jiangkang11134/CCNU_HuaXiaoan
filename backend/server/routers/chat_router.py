...
from server.utils.frontend_portal_policy import (
    ...
    ensure_frontend_file_upload_allowed,
    ensure_frontend_image_upload_allowed,
...
async def call(query: str = Body(...), meta: dict = Body(None), current_user: User = Depends(get_required_user)):
    ...
    if is_frontend_chat_user(current_user):
        raise HTTPException(status_code=403, detail="前台用户只能使用受控问答入口")
...
async def create_thread(
    ...
    """创建新对话线程 (使用新存储系统)"""
    await ensure_frontend_agent_allowed(db, current_user, thread.agent_id)
...
async def upload_tmp_attachment(
    ...
    await ensure_frontend_file_upload_allowed(db, current_user)
