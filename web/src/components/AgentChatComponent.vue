          ...
          <div class="bottom" :class="{ 'start-screen': !conversations.length }">
            ...
            <div class="message-input-wrapper">
              ...
              <AgentInputArea
                ...
                :supports-image-upload="supportsImageUpload"
                :show-send-button="shouldShowSendButton"
...
const props = defineProps({
  ...
  frontendConfig: { type: Object, default: null }
})
...
const greetingTitle = computed(() => {
  if (props.frontendConfig?.welcome_title_mode === 'custom' && props.frontendConfig?.welcome_title) {
    return props.frontendConfig.welcome_title
  }
...
// 检查当前智能体是否支持上传能力，再由前台配置拆分文件/图片入口。
const agentSupportsUpload = computed(() => {
  if (!currentAgent.value) return false
  const capabilities = currentAgent.value.capabilities || []
  return capabilities.includes('file_upload')
})
const supportsFileUpload = computed(() => {
  if (!agentSupportsUpload.value) return false
  return props.frontendConfig ? props.frontendConfig.show_file_upload === true : true
})
const supportsImageUpload = computed(() => {
  if (!agentSupportsUpload.value) return false
  return props.frontendConfig ? props.frontendConfig.show_image_upload === true : true
})
