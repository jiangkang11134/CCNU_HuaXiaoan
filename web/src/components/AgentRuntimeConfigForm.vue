...
const navigateToConfigPage = (kind) => {
  ...
  setTimeout(() => {
    switch (kind) {
      case 'knowledges':
        router.push({ path: '/back/extensions', query: { tab: 'knowledge' } })
        break
      case 'tools':
        router.push({ path: '/back/extensions', query: { tab: 'tools' } })
        break
      case 'mcps':
        router.push({ path: '/back/extensions', query: { tab: 'mcp' } })
        break
      case 'skills':
        router.push({ path: '/back/extensions', query: { tab: 'skills' } })
        break
      case 'subagents':
        router.push({ path: '/back/model-manage', query: { tab: 'agents' } })
        break
    }
  }, 100)
}
