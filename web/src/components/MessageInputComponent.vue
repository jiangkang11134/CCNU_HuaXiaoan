...
const canAcceptDroppedFiles = () => props.fileUploadEnabled && !props.disabled && !props.isLoading
const canAcceptPastedImages = () => props.imageUploadEnabled && !props.disabled && !props.isLoading
...
const handlePaste = (e) => {
  ...
  const imageFile = canAcceptPastedImages() ? getImageFileFromClipboard(e.clipboardData) : null
  if (imageFile) {
    e.preventDefault()
...
const handleDragOver = (e) => {
  if (!hasTransferFiles(e.dataTransfer)) return
  e.preventDefault()
  e.dataTransfer.dropEffect = canAcceptDroppedFiles() ? 'copy' : 'none'
  if (!canAcceptDroppedFiles()) return
  isDraggingFiles.value = true
}
