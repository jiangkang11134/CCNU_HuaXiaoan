  ...
  <div class="user-management">
              ...
              <div class="card-header">
                <div class="user-info-main">
                  ...
                  <div class="user-info-content">
                    ...
                    <div class="user-id-row">身份: {{ getBusinessRoleLabel(user.business_role) }}</div>
                  </div>
                </div>
    ...
    >
      <a-form layout="vertical" class="user-form">
        ...
        <a-form-item label="学工号 / UID" required class="form-item">
          <a-input
            v-model:value="userManagement.form.uid"
            :placeholder="isFrontendBusinessRole ? '请输入不超过12位数字学工号' : '请输入3-20位字母、数字或下划线 UID'"
            :disabled="userManagement.editMode && userManagement.form.isBuiltin"
            :maxlength="20"
          />
          <div class="help-text">
...
const filteredUsers = computed(() => {
  ...
  return userManagement.users.filter((user) => {
    ...
    return matchesKeyword && matchesDepartment && matchesRole && matchesBusinessRole
  })
})

const isFrontendBusinessRole = computed(() =>
  ['student', 'faculty'].includes(userManagement.form.businessRole)
)
...
watch(
  ...
  (role) => {
    if (role !== 'user') {
      userManagement.form.businessRole = 'system_admin'
      return
    }
    if (userManagement.form.businessRole === 'system_admin') {
      userManagement.form.businessRole = 'student'
    }
  }
)
...
watch(
  () => [
    ...
    userManagement.businessRoleFilter
  ],
  () => {
...
const handleUserFormSubmit = async () => {
  try {
    ...
    const nextUid = userManagement.form.uid.trim()
    if (!nextUid) {
      message.error('学工号或 UID 不能为空')
      return
    }
