...
export const useAgentStore = defineStore(
  ...
  () => {
    ...
    const frontendChatConfig = ref(null)
    ...
    async function fetchAgents({ includeSubagents = false } = {}) {
      ...
      try {
        ...
        let nextAgents = (response.agents || []).map(normalizeAgent)
        if (userStore.isChatOnlyUser) {
          const configResponse = await frontendChatConfigApi.getConfig()
          frontendChatConfig.value = configResponse.data
          const allowedIds = new Set(frontendChatConfig.value?.selectable_agent_ids || [])
          nextAgents = nextAgents.filter((agent) => allowedIds.has(agent.id))
          if (nextAgents.length === 0) {
            throw new Error('当前没有可用智能体，请先在后台前台配置中设置候选智能体')
          }
        }
        agents.value = sortAgents(nextAgents)
      } catch (err) {
