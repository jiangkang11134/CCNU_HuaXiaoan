...
export const useUserStore = defineStore('user', () => {
  ...
  function applyUserToken(data) {
    ...
    businessRole.value = data.business_role || ''
    isBuiltin.value = Boolean(data.is_builtin)
  ...
  async function login(credentials) {
    try {
      ...
      const portal = credentials.portal ? `?portal=${encodeURIComponent(credentials.portal)}` : ''
      const response = await fetch(`/api/auth/token${portal}`, {
        method: 'POST',
        body: formData
      })
  ...
  async function registerFrontend(payload) {
    try {
  ...
  async function getCurrentUser() {
    try {
      ...
      businessRole.value = userData.business_role || ''
      isBuiltin.value = Boolean(userData.is_builtin)
