...
const knowledgeViewItems = [
  { key: 'documents', label: '文档知识库', path: '/back/extensions?tab=knowledge' }
]
...
const navigateToDatabase = (database) => {
  router.push({ path: `/back/extensions/knowledgebase/${database.kb_id}` })
}

watch(
  () => route.path,
  (newPath) => {
    if ((newPath === '/extensions' || newPath === '/back/extensions') && route.query.tab === 'knowledge') {
