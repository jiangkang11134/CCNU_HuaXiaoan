          ...
          <div class="form-wrapper">
            ...
            <div class="login-content" :class="{ 'is-initializing': isFirstRun }">
              ...
              <div v-else class="login-form">
                <div v-if="isFrontPortal" class="portal-tabs">
                  <button
                    type="button"
                ...
                <a-form :model="activeAuthForm" @finish="handleLogin" layout="vertical">
                  ...
                  <a-form-item
                    v-if="authMode === 'login'"
                    :label="isFrontPortal ? '学工号' : '登录账号'"
                    name="loginId"
                    :rules="[{ required: true, message: isFrontPortal ? '请输入学工号' : '请输入UID或手机号' }]"
                  >
                    <a-input v-model:value="loginForm.loginId" :placeholder="isFrontPortal ? '学工号' : 'UID或手机号'">
                      <template #prefix>
                        <user-icon size="18" />
                      </template>
                    </a-input>
                  ...
                  <a-form-item
                    ...
                    :rules="[{ required: true, message: '请输入密码' }]"
                  >
                    <a-input-password v-model:value="activePassword">
                      <template #prefix>
                        <lock-icon size="18" />
                      </template>
                    </a-input-password>
                  ...
                  <a-form-item>
                    ...
                    >
                      ...
                      <span v-else>{{ authMode === 'register' ? '注册并登录' : '登录' }}</span>
                    </a-button>
                  </a-form-item>
                </a-form>
...
import { getLandingPathForRole, getPortalMode } from '@/utils/portal'
...
const portalMode = computed(() => route.meta.portalMode || getPortalMode(route.path))
const isFrontPortal = computed(() => portalMode.value === 'front')
...
const activeAuthForm = computed(() => (authMode.value === 'register' ? registerForm : loginForm))
const activePassword = computed({
  get() {
    return authMode.value === 'register' ? registerForm.password : loginForm.password
  },
  set(value) {
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (authMode.value === 'register') {
      if (!/^\d{1,12}$/.test(registerForm.uid.trim())) {
        message.error('学工号必须为不超过12位数字')
        return
      }
      ...
      await userStore.registerFrontend({
        uid: registerForm.uid.trim(),
        username: registerForm.username.trim() || null,
        business_role: registerForm.businessRole,
      ...
      await userStore.login({
        ...
        portal: portalMode.value
      })
    }

    message.success(authMode.value === 'register' ? '注册成功' : '登录成功')
    ...
    if (redirectPath === '/') {
      try {
        ...
        router.push(getLandingPathForRole(userStore.userRole))
      } catch (error) {
